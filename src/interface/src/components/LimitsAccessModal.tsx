...
export default function LimitsAccessModal({
  ...
  const detail = hasSession
    ? accessGranted
      ? 'Higher limits are enabled for this account.'
      : 'Standard production limits apply for this account.'
    : 'Sign in to see limits for your account.';
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-content" style={{ maxWidth: '420px' }}>
        ...
        <div style={{ marginTop: '8px', fontSize: '13px', color: 'var(--text-muted)', lineHeight: 1.4 }}>
          {detail}
        </div>
        ...
        <div style={{ marginTop: '14px' }}>
          ...
          <div style={{ fontSize: '12px', color: 'var(--text-primary)', lineHeight: 1.5 }}>
            {hasSession ? (
              ...
              'Sign in to see whether your account is allowlisted.'
            )}
          </div>
          ...
          {(code || expiresAt) && (
            <div style={{ marginTop: '10px', fontFamily: 'var(--font-mono)', fontSize: '11px' }}>
              {code ? (
                <div style={{ color: 'var(--text-muted)' }}>Code ignored (whitelist-only)</div>
              ) : null}
              {expiresAt ? (
                <div style={{ color: 'var(--text-muted)' }}>Expires ignored: {expiresAt}</div>
              ) : null}
            </div>
          )}
        </div>
